...
constexpr uint64_t ExpandBits64(const uint32_t a) noexcept {
...
constexpr uint64_t Encode64(const uint32_t x, const uint32_t y,
                            const uint32_t z) noexcept {
  return ExpandBits64(x) | (ExpandBits64(y) << 1) | (ExpandBits64(z) << 2);
}

constexpr uint32_t CompressBits64(const uint64_t m) noexcept {
...
/**
 * @brief Encode a point to a Morton code.
 *
 ...
 * @return Code_t
 */
constexpr Code_t PointToCode(const float x, const float y, const float z,
