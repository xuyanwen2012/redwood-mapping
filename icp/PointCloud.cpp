...
#include "SimpleICP.hpp"

PointCloud::PointCloud(Eigen::MatrixXd X)
    : X_{X}, sel_{std::vector<bool>(X.rows(), true)} {}
...
void PointCloud::SelectInRange(const Eigen::MatrixX3d &X,
...
void PointCloud::SelectNPts(const uint n) {
  ...
  if (n < sel_idx.size()) {
    ...
    auto idx_not_rounded{Eigen::VectorXd::LinSpaced(
        n, 0, static_cast<uint>(sel_idx.size()) - 1)};
    for (uint i = 0; i < n; i++) {
...
void PointCloud::EstimateNormals(const int neighbors) {
  ...
  for (uint i = 0; i < sel_idx.size(); i++) {
    ...
    Eigen::MatrixXd C =
        (centered.adjoint() * centered) / double(X_nn.rows() - 1);

    // Normal vector as eigenvector corresponding to smallest eigenvalue
    // Note that SelfAdjointEigenSolver is faster than EigenSolver for symmetric
    // matrices. Moreover, the eigenvalues are sorted in increasing order.
