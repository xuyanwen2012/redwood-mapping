...
CompressBits64(const uint64_t m) noexcept {
...
Decode64(const uint64_t m, uint32_t &x, uint32_t &y, uint32_t &z) noexcept {
  x = CompressBits64(m);
...
__host__ __device__ __forceinline__ void
CodeToPoint(const Code_t code, float &dec_x, float &dec_y, float &dec_z,
            const float min_coord, const float range) noexcept {
  constexpr uint32_t bit_scale = 0xFFFFFFFFu >> (32 - (kCodeLen / 3));
...
struct MortonEncoder {
  constexpr MortonEncoder(const float min_coord, const float range)
...
struct MortonDecoder {
  constexpr MortonDecoder(const float min_coord, const float range)
      : min_coord(min_coord), range(range) {}

  __host__ __device__ __forceinline__ float3
  operator()(const Code_t code) const {
    float dec_x, dec_y, dec_z;
    CodeToPoint(code, dec_x, dec_y, dec_z, min_coord, range);
    return make_float3(dec_x, dec_y, dec_z);
  }
